...
class IndicacaoVotacao(db.Model):
    ...
    criado_em=db.Column(db.DateTime,nullable=False,default=datetime.utcnow)
    external_response_id=db.Column(db.String(180),nullable=True,index=True)
...
with app.app_context():
    ...
    # V14: IDs externos das respostas do Google Forms evitam importação duplicada.
    try:
        with db.engine.begin() as c:
            if db.engine.dialect.name=='postgresql':
                c.exec_driver_sql('ALTER TABLE indicacao_votacao ADD COLUMN IF NOT EXISTS external_response_id VARCHAR(180)')
                c.exec_driver_sql('ALTER TABLE voto_comissao ADD COLUMN IF NOT EXISTS external_response_id VARCHAR(180)')
            else:
                for tabela in ('indicacao_votacao','voto_comissao'):
                    cols=[r[1] for r in c.exec_driver_sql(f'PRAGMA table_info({tabela})').fetchall()]
                    if 'external_response_id' not in cols:c.exec_driver_sql(f'ALTER TABLE {tabela} ADD COLUMN external_response_id VARCHAR(180)')
    except Exception as e:
        print('Aviso migração Google Forms V14:',e)
    bootstrap_usuarios()
...
def _google_call(payload):
    endpoint=os.environ.get('GOOGLE_FORMS_WEBAPP_URL','').strip()
    if not endpoint: raise RuntimeError('GOOGLE_FORMS_WEBAPP_URL não configurada')
    ...
    if not out.get('ok', True): raise RuntimeError(out.get('erro') or 'Falha no Google Forms')
    return out

def _google_form(tipo, processo, candidatos=None):
    out=_google_call({'acao':'criar_formulario','tipo':tipo,'processoId':processo.id,'titulo':processo.titulo,
      'descricao':processo.descricao or '','candidatos':candidatos or [],'maxVotos':processo.max_votos})
    return out.get('responder_url') or out.get('url') or out.get('publishedUrl')
...
@app.post('/api/votacoes/<int:pid>/sincronizar-indicacoes')
def sincronizar_indicacoes(pid):
    if not _admin():return jsonify({'detail':'Somente administrador'}),403
    x=ProcessoVotacao.query.get_or_404(pid)
    try: out=_google_call({'acao':'ler_respostas','tipo':'indicacao','processoId':pid})
    except Exception as e:return jsonify({'detail':str(e)}),502
    novas=0
    for r in out.get('respostas',[]):
        rid=str(r.get('id') or '').strip(); nome=str(r.get('nome') or '').strip()
        if not rid or not nome:continue
        if IndicacaoVotacao.query.filter_by(processo_id=pid,external_response_id=rid).first():continue
        db.session.add(IndicacaoVotacao(processo_id=pid,nome=nome,indicado_por='Google Forms',external_response_id=rid));novas+=1
    db.session.commit();d=_proc(x);d['novasImportadas']=novas;return d

@app.post('/api/votacoes/<int:pid>/sincronizar-votos')
def sincronizar_votos(pid):
    import hashlib
    if not _admin():return jsonify({'detail':'Somente administrador'}),403
    x=ProcessoVotacao.query.get_or_404(pid)
    try: out=_google_call({'acao':'ler_respostas','tipo':'votacao','processoId':pid})
    except Exception as e:return jsonify({'detail':str(e)}),502
    eleg=[c['nome'] for c in _proc(x)['candidatos'] if c['indicacoes']>=x.minimo_indicacoes];novas=0
    for r in out.get('respostas',[]):
        rid=str(r.get('id') or '').strip(); nomes=r.get('nomes') or []
        if not rid or not isinstance(nomes,list) or not nomes or len(nomes)>x.max_votos:continue
        nomes=[str(n).strip() for n in nomes if str(n).strip() in eleg]
        if not nomes:continue
        if VotoComissao.query.filter_by(processo_id=pid,external_response_id=rid).first():continue
        h=hashlib.sha256((str(pid)+'|FORM|'+rid+'|'+app.config['SECRET_KEY']).encode()).hexdigest()
        for n in nomes:db.session.add(VotoComissao(processo_id=pid,candidato=n,eleitor_hash=h,external_response_id=rid));novas+=1
    db.session.commit();d=_proc(x);d['novosVotosImportados']=novas;return d
...
def abrir_votacao(pid):
    ...
    x=ProcessoVotacao.query.get_or_404(pid)
    try:
        out=_google_call({'acao':'ler_respostas','tipo':'indicacao','processoId':pid})
        for r in out.get('respostas',[]):
            rid=str(r.get('id') or '').strip(); nome=str(r.get('nome') or '').strip()
            if rid and nome and not IndicacaoVotacao.query.filter_by(processo_id=pid,external_response_id=rid).first():db.session.add(IndicacaoVotacao(processo_id=pid,nome=nome,indicado_por='Google Forms',external_response_id=rid))
        db.session.commit()
    except Exception as e:return jsonify({'detail':'Falha ao sincronizar indicações: '+str(e)}),502
    dados=_proc(x); nomes=[c['nome'] for c in dados['candidatos'] if c['indicacoes']>=x.minimo_indicacoes]
...
def encerrar_votacao(pid):
    ...
    x=ProcessoVotacao.query.get_or_404(pid)
    try:
        out=_google_call({'acao':'ler_respostas','tipo':'votacao','processoId':pid})
        import hashlib
        eleg=[c['nome'] for c in _proc(x)['candidatos'] if c['indicacoes']>=x.minimo_indicacoes]
        for r in out.get('respostas',[]):
            rid=str(r.get('id') or '').strip(); nomes=r.get('nomes') or []
            if not rid or not isinstance(nomes,list) or VotoComissao.query.filter_by(processo_id=pid,external_response_id=rid).first():continue
            nomes=[str(n).strip() for n in nomes if str(n).strip() in eleg][:x.max_votos]
            h=hashlib.sha256((str(pid)+'|FORM|'+rid+'|'+app.config['SECRET_KEY']).encode()).hexdigest()
            for n in nomes:db.session.add(VotoComissao(processo_id=pid,candidato=n,eleitor_hash=h,external_response_id=rid))
        db.session.commit()
    except Exception as e:return jsonify({'detail':'Falha ao sincronizar votos antes do encerramento: '+str(e)}),502
    x.status='Finalizada';db.session.commit();return _proc(x)
