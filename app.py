...
def bootstrap_usuarios():
    ...
    if not admin:
        db.session.add(Usuario(email=admin_email, nome='Administrador Geral', distrito_id=None, role='admin', password_hash=generate_password_hash(admin_password)))
    else:
        admin.nome='Administrador Geral'; admin.role='admin'; admin.ativo=True
    db.session.commit()
